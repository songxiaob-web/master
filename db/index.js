...
/**
* @description: 查询数据 
* @param {} options：可选参数? 难道不应该是 function ?  Array 传值为 [ name,age,gender.... ]
* @param {Function} callback :（req,results）=>{}
* options: 没有callback  options => function | Array |  sql语句( 字符串 )
*
*/
    ...
    if (!isConnect) {
        this.connect(err => {
            ...
            if (!callback) {
                ...
                // this.name 数据库的名字   options => 条件
                str = `select * from ${this.name} where ${options}`;
            };
            //console.log(str);
            // 获取到真是的mysql数据后 =>  connect.query 查询获取数据
...
/**
 * 
 * @param {Object} obj  must be Object or Array 
 * @param {*} callback 
 */
Model.prototype.insertObj = function (obj, callback) {
    if (!obj || typeof obj !== 'object') throw new Error('type error obj is must be object or Array object')
    let keys = [];
    let values = '';
    // { name:1,age:2 }
    // obj key =>  name, age
    // insert into student ( a,b,c,d ) values(1,2,3,4)
    for (var key in obj) {
        keys.push(key);
        values += `"${obj[key]}",`;
    };
    // values :  "1","2" => 会把中间的引号和逗号也当做一个字符来处理
...
* @param {Function} callback :（req,results）=>{}
* update student set name = 1 where name='' ;
*/
